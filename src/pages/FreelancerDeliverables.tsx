...
const FreelancerDeliverables = () => {
  const navigate = useNavigate();

  const allDeliverables = freelancerProjects.flatMap((project) =>
        ...
        <div className="bg-card border border-border rounded-xl shadow-card overflow-hidden">
              ...
              <tbody>
                {allDeliverables.map((d) => {
                  ...
                  return (
                    <tr
                      key={d.id}
                      onClick={() => navigate(`/freelancer/project/${d.projectId}/deliverable/${d.id}`)}
          ...
          <div className="md:hidden divide-y divide-border">
            {allDeliverables.map((d) => {
              const Icon = typeIcons[d.previewType] || FileText;
              return (
                <div key={d.id} onClick={() => navigate(`/freelancer/project/${d.projectId}/deliverable/${d.id}`)} className="p-4 hover:bg-accent/50 transition-colors cursor-pointer">
...
export default FreelancerDeliverables;